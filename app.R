...
#setwd("C:/Users/ayeaz/Downloads/titanic/Mushu")

output.df$Age <- as.integer(output.df$Age)


#Define UI

ui <- fluidPage(theme = shinytheme("superhero"),
                navbarPage("Titanic Survival Prediction",
                           tabPanel("App",
                                    sidebarLayout(
                                    sidebarPanel
                                    (tags$h3("Select Passenger ID")),
                                    selectInput(inputId = "PassengerId", label = "Passenger ID:",
                                    choices = titanic.test$PassengerId))
                           )), 
                ...
                tabPanel("About",
                         ...
                         div(includeHTML("Titanic-Report.html")
                             ))
    )
...
server <- function(input, output) {
    output$table <- renderTable({output.df[output.df$PassengerId==input$PassengerId,]})
